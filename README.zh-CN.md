...
- **流量策略**：路由/DNS 规则搜索、取反快捷开关、sing-box 1.13 接口感知地址匹配、Inline headless rule 结构化编辑、移动端更紧凑的规则卡片、DNS 服务器搜索；规则名称/描述独立持久化；规则集更新错误码诊断；应用前 dry-run 校验；规则/服务器路径深链打开对话框，支持对话框内 dry-run 校验
