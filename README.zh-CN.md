...
- **高级配置**：Endpoints（含 WireGuard Peer 可视化编辑）、NTP 时间同步、Experimental、完整内核 JSON（路径级 diff）、本地 JSON 导入/导出、只读 Tag/出站/DNS/规则集引用预检；raw/endpoints/NTP/experimental dry-run 校验；保存错误跳转
...
1. 登录面板，轮换管理员密码。
2. **订阅 / 节点**：添加公网 HTTP(S) 订阅 URL 或导入单节点；本机/私网源和不安全重定向会被拦截。订阅会按各自间隔后台刷新，全局间隔作为旧数据回退。按需配置 URLTest（可继承全局默认）。订阅下载限制为 16 MiB，刷新或配置同步失败会展示可操作错误码。
3. **入站 / 出站**：可一键安装 mixed（1080）与 TUN 模板，或自建入站；出站可绑定订阅组 selector / urltest，或直连/阻断等。
4. **路由 / DNS / NTP / Experimental**：用表单维护规则和时间同步；可一键安装默认规则与 Clash API。
