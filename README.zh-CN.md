...
- **配置安全**：成功应用会保留有界本地快照；仪表盘会先展示路径级差异，再恢复已知可用的 sing-box 配置，并继续经过校验、重启与失败自动回滚
...
- **高级配置**：Endpoints（含 WireGuard Peer 可视化编辑）、NTP 时间同步、Experimental、完整内核 JSON（路径级 diff）、本地 JSON 导入/导出、只读 Tag/出站/DNS/规则集引用预检，以及支持搜索、状态筛选和快照恢复的完整配置历史审计页；raw/endpoints/NTP/experimental dry-run 校验；保存错误跳转
