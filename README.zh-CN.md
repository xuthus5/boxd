...
- **代理配置**：入站 / 出站结构化表单、列表搜索、复制 tag、故障闭环 deep-link（连接/日志/节点），高级 JSON 路径级差异、编辑对话框 dry-run 校验
- **流量策略**：路由/DNS 规则搜索、取反快捷开关、移动端更紧凑的规则卡片、DNS 服务器搜索；规则名称/描述独立持久化；规则集更新错误码诊断；应用前 dry-run 校验
