...
- **仪表盘**：内核启停/重启、面板就绪状态与失败重试、快速上手清单、sing-box 语义配置诊断（拓扑/特性摘要与编辑器深链）、规则集文件/缓存新鲜度、逐规则集自动更新诊断与批量刷新、运行健康（失败订阅/不稳节点/配置应用失败告警）、配置应用时间线、全局出口/Clash 模式（延迟诊断）、实时流量/最近日志、内存
