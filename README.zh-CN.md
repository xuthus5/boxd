...
- **高级配置**：Endpoints（含 WireGuard Peer 可视化编辑）、X.509 证书信任库、NTP 时间同步、Experimental、完整内核 JSON（路径级 diff）、本地 JSON 导入/导出、只读 Tag/出站/DNS/规则集引用预检，以及支持搜索、状态摘要、筛选 JSON 导出、精确时间、完整错误和快照恢复的完整配置历史审计页；raw/endpoints/certificate/NTP/experimental dry-run 校验；保存错误跳转
...
4. **路由 / DNS / 证书 / NTP / Experimental**：用表单维护规则、信任库和时间同步；可一键安装默认规则与 Clash API。
