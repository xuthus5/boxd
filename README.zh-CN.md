...
- **仪表盘**：内核启停/重启、面板就绪状态与失败重试、快速上手清单、运行健康（失败订阅/不稳节点/配置应用失败告警 + 流/应用错误 densify）、配置应用时间线（错误码诊断、含按编辑入口标注的 dry-run 校验）、卡片级查询失败 densify（出口选择/Clash 模式/应用时间线）、全局出口/Clash 模式（延迟诊断）、实时流量/最近日志（流错误诊断）、内存
...
- **高级配置**：Endpoints（含 WireGuard Peer 可视化编辑）、NTP 时间同步、Experimental、完整内核 JSON（路径级 diff）、本地 JSON 导入/导出、只读 Tag/出站/DNS/规则集引用预检，以及支持搜索、状态摘要、筛选 JSON 导出、精确时间、完整错误和快照恢复的完整配置历史审计页；raw/endpoints/NTP/experimental dry-run 校验；保存错误跳转
