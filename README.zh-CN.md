...
- **节点与订阅**：Clash YAML / base64 订阅、失败优先与重试、流量/到期、订阅卡片 deep-link 到节点/日志、移动端更紧凑的工具栏/卡片、节点搜索、延迟色阶、运行时分组、测速（错误码提示与可复制诊断）
- **运行观测**：内核/应用日志（更紧凑筛选/操作区、长列表虚拟滚动）、活跃连接（紧凑工具栏、搜索、按出口/规则聚合、分组下钻与关闭（失败诊断）、长列表虚拟滚动）
- **高级配置**：Endpoints、Experimental、完整内核 JSON（路径级 diff）；raw/endpoints/experimental dry-run 校验；保存错误跳转
- **认证**：登录失败展示错误码/可操作提示与一键复制诊断
- **页面加载诊断**：仪表盘/代理/策略/节点订阅/设置/高级配置等页面的查询失败统一展示错误码、可操作提示与复制诊断
