...
archive_name="boxui_${version}_linux_amd64.tar.gz"
stage_dir=$(mktemp -d)
...
install -m 0700 bin/boxui "$stage_dir/boxui"
...
install -m 0600 deploy/boxui.service "$stage_dir/boxui.service"
install -m 0600 deploy/boxui.env.example "$stage_dir/boxui.env.example"
...
printf '%s\n' \
  'Corresponding source for this binary is available from the BoxUI Git tag:' \
  "$version" \
  '' \
