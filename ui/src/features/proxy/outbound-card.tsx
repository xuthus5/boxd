    ...
    <article aria-labelledby={titleId}>
      <Card size="sm" className="h-full">
        <CardHeader className="min-w-0 gap-1.5">
          ...
          <CardDescription className="truncate">{server ?? t("proxy.outbound.groupOutbound")}</CardDescription>
        ...
        <CardContent className="flex flex-col gap-2">
          <div className="flex flex-wrap gap-1.5">
            <Badge>{type}</Badge>
            {tls ? <Badge variant="secondary">TLS</Badge> : null}
            {transport && transport !== "—" ? <Badge variant="outline">{transport}</Badge> : null}
          </div>
          {members.length ? (
            <p className="line-clamp-2 text-xs text-muted-foreground sm:text-sm">
              {t("proxy.outbound.members", { members: members.join(", ") })}
            </p>
          ) : null}
          {item.detour ? (
