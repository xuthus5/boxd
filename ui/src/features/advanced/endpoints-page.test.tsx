...
describe("endpoints page", () => {
  ...
  it("adds a wireguard endpoint from the visual editor and saves", async () => {
    ...
    expect(dialog).toHaveClass("p-3")
    expect(dialog).toHaveClass("max-h-[calc(100dvh-1rem)]")
    expect(within(dialog).getByRole("button", { name: "保存" })).toHaveClass("h-8")
    fill("Tag", "wg-new", dialog)
    fill("本端地址", "10.8.0.2/32", dialog)
    fill("私钥", "secret-key", dialog)
    await user.click(within(dialog).getAllByRole("button", { name: "新增 Peer" })[0])
    const peerDialog = await screen.findByRole("dialog", { name: "新增 WireGuard Peer" })
    fill("Peer 公钥", "peer-public-key", peerDialog)
    fill("允许的 IP", "0.0.0.0/0", peerDialog)
    await user.click(within(peerDialog).getByRole("button", { name: "保存" }))
