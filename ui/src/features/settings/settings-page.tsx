...
function AccountCard({ defaultPassword, jwt }: { defaultPassword: boolean; jwt: { masked: string; present: boolean; length: number } }) {
  ...
  return <Card><CardHeader><CardTitle>{t("settings.accountTitle")}</CardTitle><CardDescription>{t("settings.accountDescription")}</CardDescription></CardHeader><CardContent className="flex flex-col gap-4">
    {defaultPassword ? <Alert variant="destructive"><AlertTitle>{t("settings.defaultPasswordTitle")}</AlertTitle><AlertDescription>{t("settings.defaultPasswordDescription")}</AlertDescription></Alert> : null}
...
function RuntimeSettingsCard({ url, enabled }: { url: string; enabled: boolean }) {
  ...
  return <Card><CardHeader><CardTitle>{t("settings.runtimeTitle")}</CardTitle><CardDescription>{t("settings.runtimeDescription")}</CardDescription></CardHeader><CardContent><FieldGroup><Field><FieldLabel htmlFor="test-url">{t("settings.testURL")}</FieldLabel><Input id="test-url" value={testURL} onChange={(event) => setTestURL(event.target.value)} /><Button onClick={() => saveURL.mutate()}>{t("settings.saveTestURL")}</Button></Field><Field orientation="horizontal"><FieldLabel htmlFor="autostart">{t("settings.autostart")}</FieldLabel><Switch id="autostart" checked={autostart} onCheckedChange={saveAutostart} /></Field></FieldGroup></CardContent></Card>
}
