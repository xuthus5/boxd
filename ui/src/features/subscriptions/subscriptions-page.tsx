    ...
    <article aria-label={item.name}>
      <Card size="sm">
        ...
        <CardFooter className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          ...
          <Button size="sm" variant="outline" onClick={onRefresh}>{t("subscriptions.refresh")}</Button>
          <ConfirmAction
          ...
          ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {sortSubscriptions(query.data).map((item) => (
              <SubscriptionItem
                ...
                onRefresh={() => action(api.subscriptions.refresh(item.id), t("subscriptions.refresh"))}
