...
describe("api endpoint coverage", () => {
  it("maps every backend operation to an HTTP request", async () => {
    ...
    await Promise.all([
      ...
      api.config.applyHistory(), api.config.restoreApplyHistory("event/one"), api.config.installDNS(), api.config.installRuleSets(), api.config.installOutbounds(), api.config.installInbounds(), api.config.installExperimental(), api.config.installRoute(),
