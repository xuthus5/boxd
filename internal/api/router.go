...
func NewRouter(
	...
	r.Route("/api/config", func(r chi.Router) {
		...
		r.Get("/diagnostics", configHandler.ConfigDiagnostics)
		r.Put("/raw", configHandler.UpdateRawConfig)
		r.Post("/validate", configHandler.ValidateConfig)
		r.Get("/setup", configHandler.GetSetup)
		r.Post("/setup/preview", configHandler.PreviewSetup)
		r.Post("/setup/apply", configHandler.ApplySetup)
		r.Get("/apply-history", configHandler.ListConfigApplyHistory)
		r.Post("/apply-history/{id}/restore", configHandler.RestoreConfig)
		r.Get("/apply-history/{id}/snapshot", configHandler.GetConfigSnapshot)
	...
	r.Route("/api/runtime", func(r chi.Router) {
		...
		r.Post("/dns/probe", runtimeHandler.ProbeDNS)
		r.Post("/dns/probe-batch", runtimeHandler.ProbeDNSBatch)
