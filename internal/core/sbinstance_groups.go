...
// CloseConnectionsByProcess closes active connections matched by the process path/name.
func (s *SBInstance) CloseConnectionsByProcess(process string) int {
	s.mu.Lock()
	traffic := s.Traffic
	s.mu.Unlock()
	if traffic == nil {
	...
	return traffic.CloseConnsByProcess(process)
}
