...
- **Advanced**: Endpoints with visual WireGuard peer editing, NTP time sync, Experimental, full kernel JSON with path-level diff, local JSON import/export, read-only Tag/outbound/DNS/rule-set preflight; dry-run validate on raw/endpoints/NTP/experimental; sticky save-error jump
