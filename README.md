...
- **Traffic policy**: route/DNS rules with search, invert toggles, sing-box 1.13 interface-aware address matches, structured inline headless rule sets, denser mobile cards, DNS servers with search, health probe, and error-code diagnostics; rule name/description stored by boxd; rule-set update error-code diagnostics; dry-run validate before apply; item path deep-links into rule/server dialogs with dialog dry-run validate
