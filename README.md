...
- **Advanced**: Endpoints with visual WireGuard peer editing, X.509 certificate trust stores, sing-box CCM/OCM/DERP/Resolved/SSM API services, NTP time sync, Experimental, full kernel JSON with path-level diff, local JSON import/export, read-only Tag/outbound/DNS/rule-set preflight, and searchable config-history audit/restore; dry-run validate on raw/endpoints/certificate/services/NTP/experimental; sticky save-error jump
...
4. **Route / DNS / Certificates / Services / NTP / Experimental**: edit policies, trust stores, auxiliary kernel services, and time sync in forms; install common defaults; one-click enable Clash API.
