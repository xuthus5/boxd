...
- **Dashboard**: start/stop/restart kernel, setup checklist with failed-subscription previews, live health with failed-sub/unstable-node alerts, config apply timeline with error-code diagnostics and copyable failures, densified card-level query failures (proxy selector / Clash mode / apply timeline), global outbound/Clash mode switch with latency diagnostics, live traffic, memory, recent logs
