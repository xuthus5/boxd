...
- **Advanced**: Endpoints with visual WireGuard peer editing, X.509 certificate trust stores, sing-box CCM/OCM/DERP/Resolved/SSM API services, kernel logging, NTP time sync, Experimental, full kernel JSON with path-level diff, local JSON import/export, read-only Tag/outbound/DNS/rule-set preflight, and searchable config-history audit/restore; dry-run validate on raw/endpoints/certificate/services/log/NTP/experimental; sticky save-error jump
