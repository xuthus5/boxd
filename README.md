...
- **Dashboard**: start/stop/restart kernel, setup checklist with failed-subscription previews, live health with failed-sub/unstable-node alerts, config apply timeline with copyable failures, global outbound/Clash mode switch, live traffic, memory, recent logs
- **Proxy config**: structured inbound/outbound forms (TLS/Reality, transports), numeric field validation, list search, tag copy, ops deep-links to connections/logs/nodes, path-level save errors, plus Advanced JSON with path-level save diff
...
- **Nodes & subscriptions**: Clash YAML + base64 subscriptions, traffic/expiry, failed-first + retry with error-code hints and copy/open diagnostics, subscription deep-links to nodes/logs, denser mobile toolbars/cards, node search, latency color bands + history sparkline/detail chart, runtime groups, probes
- **Observability**: kernel/app logs with copy/export, denser mobile filters, and virtualized long lists; TCP/UDP connections with source/network/inbound/process, compact toolbar, search/sort/export, optimistic close against live SSE, outbound/rule/process group views (virtualized), and group drill-down/close
