...
- 本项目禁止使用 SuperPowers 技能体系及其工作流。
- 项目需求澄清、需求设计与方案评审统一使用 `grill-me` 技能。
...
- 以下操作仍必须确认：删除文件、大规模重构、shared contract / schema / shared types、根配置 / CI / 依赖 / 环境模板、数据库 / 持久化变更、基础设施或越界改动；“自动本地部署规则”明确授权的 BoxUI 构建与 `boxui.service` 重启除外。
- 常规本地 Git 操作（如 `status`、`diff`、`add`、`commit`）默认已授权；远程操作、强制操作、历史重写及破坏性 Git 操作仍须明确确认。
...
- 任务实际修改仓库文件且完成相关验证后，必须在最终回复前自动创建本地 Git 提交。
- 仅暂存并提交当前任务直接相关的文件，不得夹带用户已有的无关改动。
- 只读任务或没有文件变化时不创建空提交。
- 提交失败时必须如实报告，且不得将任务表述为已完整收尾。
- 自动提交不包含 `push`、创建 PR 或其他远程操作；此类操作仍须用户明确授权。

### 自动本地部署规则

- 任务实际修改仓库文件并成功创建本地 Git 提交后，必须在最终回复前部署该提交对应的 BoxUI 二进制。
- 使用仓库正式构建流程重新构建前端与 Go 二进制，安全替换 `/usr/local/bin/boxui`，然后重启 `boxui.service`。
- `/usr/local/bin/boxui` 必须保持 `root:boxui` 属主与 `0750` 权限，确保服务用户可以执行且不能写入；不得安装为服务用户无法执行的 `0700 root:root`。
- 部署后必须验证 systemd 单元处于 `active (running)`、运行进程使用新二进制，并检查本次启动日志没有启动失败。
- 构建或部署失败时必须如实报告，不得将任务表述为已完整收尾；不得因自动部署扩大到其他服务或远程环境。
- 只读任务、没有文件变化或没有创建提交时不执行部署。
