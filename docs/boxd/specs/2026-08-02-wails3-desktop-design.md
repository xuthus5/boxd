...
3. **M3 实时流与全页面**：SSE → Events 迁移，其余页面 1:1 接入 desktop transport。
4. **M4 原生能力**：托盘、自启、单实例、文件对话框、通知、深链、代理切换、快捷键。
5. **M5 打包**：wails3 package 产出 deb/rpm/AppImage + .desktop，CI 集成。
